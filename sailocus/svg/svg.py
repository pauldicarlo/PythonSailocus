...
from sailocus.sail.center_of_effort import Sail, CenterOfEffort
...
class SVG():
    ...
    def createSailSVG(self, sail:Sail, path_to_file: str, write_file: bool, margin_off_set= Point(0,0)) -> svgwrite.Drawing:
        ...
        # coe has all of the following lines and coe calculated.  
        ...
        transform_group.add(dwg.line(coe.triangles[0].centroid, coe.triangles[1].centroid, stroke='red', stroke_width=1))
        # then get the lines perpendicular to that line between centroids...
        for line_segment in coe.lines_perpendicular_to_centroid_line_segments:
        ...
        transform_group.add(dwg.line(
            coe.lines_perpendicular_to_centroid_line_segments[0].point_b,
            coe.lines_perpendicular_to_centroid_line_segments[1].point_b,
        ...
        # the intersection of which gives us coe.center_of_effort which has already been calculated

        transform_group.add(dwg.circle(center=(coe.center_of_effort), r=2, fill='blue', stroke='black', stroke_width=1))
        ...
        if coe is None:
            raise ValueError("coecannot be None")
        if coe.center_of_effort is None:
            raise ValueError("coecannot.center_of_effort be None")
        ...
        labelPoint(dwg, text_group, [sail.POINT_NAME_COE +  ': ' + str(coe.center_of_effort)], 
                   Point(coe.center_of_effort.x, -coe.center_of_effort.x), 'black', '20px' )

        relocation_group = dwg.g(transform="translate("+str(margin_off_set.x) +",-"+str(margin_off_set.y)+")")
